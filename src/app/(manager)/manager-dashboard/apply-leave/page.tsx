...
export default function ApplyLeavePage() {
  ...
  // Apply Form State
  const defaultManager = managerProfile?.reportingManager || "Executive Committee";
  ...
  const myLeaves = useMemo(() => {
    const tlId = managerProfile?.id || "emp-104";
    const tlName = managerProfile?.name || "Amira Patel";

    return leaveRequests
      .filter((r) => r.employeeId === tlId || r.employeeName === tlName)
      .sort((a, b) => new Date(b.appliedAt).getTime() - new Date(a.appliedAt).getTime());
  }, [leaveRequests, managerProfile]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      applyLeaveRequest({
        employeeId: managerProfile?.id || "emp-104",
        employeeName: managerProfile?.name || "Amira Patel",
        employeeAvatar: managerProfile?.avatar || "AP",
